...
function createUserMessageInSession (string $content, string $type, Core\Session $session) 
{
    $session->setSessionVariable('user_message', ['content' => $content, 'type' => $type]);
}

// Creates admin message in session
function createAdminMessageInSession (string $content, string $type, Core\Session $session) 
{
...
function isCsrfIncorrect(Core\Session $session) 
{
